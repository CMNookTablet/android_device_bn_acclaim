 ...
 */
struct buffer_remix {
	void (*remix_func) (struct buffer_remix * data, void *buf,
			    size_t frames);
 ...
 */
static void remove_channels_from_buf(struct buffer_remix *data, void *buf,
				     size_t frames)
{
	...
	if (out_frame >= in_frame) {
		ALOGE
		    ("BUG: remove_channels_from_buf() can not add channels to a buffer.\n");
		return;
	}
...
{
	struct buffer_remix *br =
	    (struct buffer_remix *)malloc(sizeof(struct buffer_remix));
	...
	if (in->buffer) {
		size_t chans =
		    (br->in_chans >
		     br->out_chans) ? br->in_chans : br->out_chans;
		free(in->buffer);
		in->buffer =
		    malloc(in->config.period_size * br->sample_size * chans);
		if (!in->buffer)
...
static int start_call(struct omap_audio_device *adev)
{
	...
	if (adev->modem) {
		pcm_config_vx.rate =
		    adev->wb_amr ? VX_WB_SAMPLING_RATE : VX_NB_SAMPLING_RATE;
	} else {
	...
	if (adev->pcm_modem_dl == NULL) {
		adev->pcm_modem_dl =
		    pcm_open(0, PORT_MODEM, PCM_OUT, &pcm_config_vx);
		if (!pcm_is_ready(adev->pcm_modem_dl)) {
			ALOGE("cannot open PCM modem DL stream: %s",
	...
	if (adev->pcm_modem_ul == NULL) {
		adev->pcm_modem_ul =
		    pcm_open(0, PORT_MODEM, PCM_IN, &pcm_config_vx);
		if (!pcm_is_ready(adev->pcm_modem_ul)) {
			ALOGE("cannot open PCM modem UL stream: %s",
			      pcm_get_error(adev->pcm_modem_ul));
			goto err_open_ul;
		}
	}
...
static void set_eq_filter(struct omap_audio_device *adev)
{
	...
	if ((adev->mode == AUDIO_MODE_IN_CALL) && dl1_eq_applicable &&
		...
		mixer_ctl_set_enum_by_string(adev->mixer_ctls.dl1_eq,
					     MIXER_4KHZ_LPF_0DB);
	else
		mixer_ctl_set_enum_by_string(adev->mixer_ctls.dl1_eq,
					     MIXER_FLAT_RESPONSE);
...
void audio_set_wb_amr_callback(void *data, int enable)
{
	...
	if (EDEADLK == trylock)
		ALOGV("%s: WB AMR callback calls in a deadlock situation",
		      __FUNCTION__);
	if (EBUSY == trylock) {
...
static void set_incall_device(struct omap_audio_device *adev)
{
	...
	if (!strcmp(prop, "Yes")) {
		ril_set_call_audio_path(&adev->ril,
			      ...
			      int headset_mic_on, int sub_mic_on)
{
	...
	if (adev->mode == AUDIO_MODE_IN_CALL) {
		...
		if (adev->board_type == OMAP5_SEVM) {
			volume =
			    DB_TO_ABE_GAIN(main_mic_on ?
					   VOICE_CALL_OMAP5_SEVM_MAIN_MIC_VOLUME
					   : (headset_mic_on ?
					      VOICE_CALL_OMAP5_SEVM_HEADSET_MIC_VOLUME
					      : (sub_mic_on ?
						 VOICE_CALL_OMAP5_SEVM_MAIN_MIC_VOLUME
						 : 0)));
		} else {
			volume =
			    DB_TO_ABE_GAIN(main_mic_on ?
					   VOICE_CALL_MAIN_MIC_VOLUME
					   : (headset_mic_on ?
					      VOICE_CALL_HEADSET_MIC_VOLUME
					      : (sub_mic_on ?
						 VOICE_CALL_SUB_MIC_VOLUME :
						 0)));
		}
		...
		case AUDIO_SOURCE_MIC:	/* general capture */
			if ((adev->board_type == OMAP4_BLAZE) ||
				...
				volume =
				    DB_TO_ABE_GAIN(main_mic_on ?
						   CAPTURE_MAIN_MIC_VOLUME
						   : (headset_mic_on ?
						      CAPTURE_HEADSET_MIC_VOLUME
						      : (sub_mic_on ?
							 CAPTURE_SUB_MIC_VOLUME
							 : 0)));
			} else if (adev->board_type == OMAP4_TABLET) {
				volume =
				    DB_TO_ABE_GAIN(main_mic_on ?
						   CAPTURE_DIGITAL_MIC_VOLUME
						   : (headset_mic_on ?
						      CAPTURE_HEADSET_MIC_VOLUME
						      : (sub_mic_on ?
							 CAPTURE_SUB_MIC_VOLUME
		...
		case AUDIO_SOURCE_CAMCORDER:
			volume =
			    DB_TO_ABE_GAIN(main_mic_on ?
					   CAMCORDER_MAIN_MIC_VOLUME
					   : (headset_mic_on ?
					      CAMCORDER_HEADSET_MIC_VOLUME
					      : (sub_mic_on ?
						 CAMCORDER_SUB_MIC_VOLUME :
						 0)));
			break;
		...
		case AUDIO_SOURCE_VOICE_RECOGNITION:
			volume =
			    DB_TO_ABE_GAIN(main_mic_on ?
					   ...
					   : (headset_mic_on ?
					      VOICE_RECOGNITION_HEADSET_MIC_VOLUME
					      : (sub_mic_on ?
						 VOICE_RECOGNITION_SUB_MIC_VOLUME
						 : 0)));
			break;

		case AUDIO_SOURCE_VOICE_COMMUNICATION:	/* VoIP */
			volume =
			    DB_TO_ABE_GAIN(main_mic_on ? VOIP_MAIN_MIC_VOLUME
					   : (headset_mic_on ?
					      VOIP_HEADSET_MIC_VOLUME
					      : (sub_mic_on ?
						 VOIP_SUB_MIC_VOLUME : 0)));
			break;
	...
	for (channel = 0; channel < 2; channel++) {
		if ((adev->board_type == OMAP4_BLAZE) ||
			...
			mixer_ctl_set_value(adev->mixer_ctls.amic_ul_volume,
			   ...
			   || (adev->board_type == OMAP4_ACCLAIM)) {
			if (headset_mic_on)
				mixer_ctl_set_value(adev->
						    mixer_ctls.amic_ul_volume,
						    channel, volume);
			else
				mixer_ctl_set_value(adev->
						    mixer_ctls.dmic1_ul_volume,
						    channel, volume);
		}
	}
}
...
static void set_output_volumes(struct omap_audio_device *adev)
{
	...
	speaker_volume =
	    adev->mode ==
	    AUDIO_MODE_IN_CALL ? VOICE_CALL_SPEAKER_VOLUME :
	    NORMAL_SPEAKER_VOLUME;
	headset_volume =
	    adev->out_device & AUDIO_DEVICE_OUT_WIRED_HEADSET ? HEADSET_VOLUME :
	    HEADPHONE_VOLUME;
...
static void select_mode(struct omap_audio_device *adev)
{
	...
	if (adev->mode == AUDIO_MODE_IN_CALL) {
		...
		if (!adev->in_call) {
			...
			if (adev->modem) {
				ril_set_call_clock_sync(&adev->ril,
							SOUND_CLOCK_START);
			}
			start_call(adev);
...
static void select_output_device(struct omap_audio_device *adev)
{
	...
	if (adev->mode == AUDIO_MODE_IN_CALL) {
		...
		for (channel = 0; channel < 2; channel++)
			voice_ul_volume[channel] =
			    mixer_ctl_get_value(adev->
						mixer_ctls.voice_ul_volume,
						channel);
		mixer_ctl_set_value(adev->mixer_ctls.voice_ul_volume, channel,
				    0);
	...
	set_route_by_array(adev->mixer, codec_output_controls,
			   headset_on | headphone_on | speaker_on);
	...
	if (adev->mode == AUDIO_MODE_IN_CALL) {
		if (bt_on)
			...
			if ((adev->board_type == OMAP4_BLAZE) ||
			    (adev->board_type == OMAP5_SEVM)) {
				mixer_ctl_set_enum_by_string(adev->
							     mixer_ctls.left_capture,
							     (earpiece_on
							      || headphone_on) ?
							     MIXER_MAIN_MIC
							     : (headset_on ?
	...
	mixer_ctl_set_value(adev->mixer_ctls.sidetone_capture, 0,
			    sidetone_capture_on);
...
static void select_input_device(struct omap_audio_device *adev)
{
	...
	if (bt_on)
		...
		if (adev->board_type == OMAP5_SEVM) {
			...
			set_route_by_array(adev->mixer, mm_ul2_fmradio_digital,
					   1);
		} else {
		...
		if ((adev->board_type == OMAP4_BLAZE) ||
			...
			if (headset_on)
				...
				set_route_by_array(adev->mixer, mm_ul2_dmic0,
						   1);
				//hw_is_stereo_only = 1;
			} else {
				set_route_by_array(adev->mixer, mm_ul2_dmic0,
...
//            ALOGE(">>> [ASoC]select_input_device:: devices==%d, headset_on==%d, main_mic_on==%d, sub_mic_on==%d\n", adev->devices, headset_on, main_mic_on, sub_mic_on);
			...
			mixer_ctl_set_enum_by_string(adev->
						     ...
						     : (headset_on ?
							MIXER_HS_MIC : "Off"));
			set_route_by_array(adev->mixer, codec_input_controls,
					   1);
		}
	}
...
static int check_input_parameters(uint32_t sample_rate, int format,
				  int channel_count)
{
	LOGFUNC("%s(%d, %d, %d)", __FUNCTION__, sample_rate, format,
		channel_count);
				    ...
				    int channel_count)
{
	...
	size =
	    (pcm_config_mm_ul.period_size * sample_rate) /
	    pcm_config_mm_ul.rate;
	size = ((size + 15) / 16) * 16;
...
static struct echo_reference_itfe *get_echo_reference(struct omap_audio_device
						      *adev,
						      ...
						      uint32_t sampling_rate)
{
	...
	if (adev->active_output != NULL) {
		struct audio_stream *stream =
		    &adev->active_output->stream.common;
		uint32_t wr_channel_count =
		    popcount(stream->get_channels(stream));
		...
		int status = create_echo_reference(AUDIO_FORMAT_PCM_16_BIT,
						   ...
						   &adev->echo_reference);
		if (status == 0)
			add_echo_reference(adev->active_output,
					   adev->echo_reference);
	}
			      ...
			      struct echo_reference_buffer *buffer)
{
	...
	status =
	    pcm_get_htimestamp(out->pcm, &kernel_frames, &buffer->time_stamp);
	if (status < 0) {
...
static size_t out_get_buffer_size(const struct audio_stream *stream)
{
	...
	/* take resampling into account and return the closest majoring
	   ...
	   be a multiple of 16 frames */
	size_t size =
	    (SHORT_PERIOD_SIZE * DEFAULT_OUT_SAMPLING_RATE) / out->config.rate;
	size = ((size + 15) / 16) * 16;
...
static int out_set_parameters(struct audio_stream *stream, const char *kvpairs)
{
	...
	ret =
	    str_parms_get_str(parms, AUDIO_PARAMETER_STREAM_FM_ROUTING, value,
			      sizeof(value));
	if (ret >= 0) {
		val = atoi(value);
...
static uint32_t out_get_latency(const struct audio_stream_out *stream)
{
	...
	LOGFUNC("%s(%p)", __FUNCTION__, stream);
	return (SHORT_PERIOD_SIZE * PLAYBACK_PERIOD_COUNT * 1000) /
	    out->config.rate;
}
...
do_over:
	...
	do {
		...
		if (pcm_get_htimestamp
		    (out->pcm, (unsigned int *)&kernel_frames, &time_stamp) < 0)
			break;
		kernel_frames = pcm_get_buffer_size(out->pcm) - kernel_frames;
		if (kernel_frames > out->write_threshold) {
			unsigned long time = (unsigned long)
			    (((int64_t) (kernel_frames - out->write_threshold) *
			      1000000) / MM_FULL_POWER_SAMPLING_RATE);
			      ...
			      struct echo_reference_buffer *buffer)
{
	 ...
	 * in current buffer */
	buf_delay =
	    (long)(((int64_t) (in->frames_in + in->proc_frames_in) * 1000000000)
		   / in->config.rate);
	...
	ALOGV("get_capture_delay time_stamp = [%ld].[%ld], delay_ns: [%d],"
	      ...
	      kernel_frames, in->frames_in, in->proc_frames_in, frames);
...
static int32_t update_echo_reference(struct omap_stream_in *in, size_t frames)
{
	...
	ALOGV
	    ("update_echo_reference, frames = [%d], in->ref_frames_in = [%d],  "
	     "b.frame_count = [%d]", frames, in->ref_frames_in,
	...
	if (in->ref_frames_in < frames) {
		...
		b.raw =
		    (void *)(in->ref_buf +
			     in->ref_frames_in * in->config.channels);
		...
		if (in->echo_reference->read(in->echo_reference, &b) == 0) {
			...
			ALOGV("update_echo_reference: in->ref_frames_in:[%d], "
			      ...
			      in->ref_frames_in, in->ref_buf_size, frames,
			      b.frame_count);
		}
	} else
		ALOGW
		    ("update_echo_reference: NOT enough frames to read ref buffer");
	return b.delay_ns;
}
			   ...
			   struct resampler_buffer *buffer)
{
	...
	in = (struct omap_stream_in *)((char *)buffer_provider -
				       offsetof(struct omap_stream_in,
						buf_provider));
	...
	if (in->frames_in == 0) {
		in->read_status = pcm_read(in->pcm,
					   ...
					   hw_frame_size);
		if (in->read_status != 0) {
			ALOGE("get_next_buffer() pcm_read error %d",
			      in->read_status);
			buffer->raw = NULL;
			buffer->frame_count = 0;
...
static ssize_t read_frames(struct omap_stream_in *in, void *buffer,
			   ssize_t frames)
{
	ssize_t frames_wr = 0;
	size_t frame_size;
	...
	if (in->remix_at_driver)
		frame_size =
		    in->remix_at_driver->out_chans *
		    in->remix_at_driver->sample_size;
	else
...
frame_count:			frames_rd,
			...
			if (buf.raw != NULL) {
				memcpy((char *)buffer +
				       ...
				       buf.raw, buf.frame_count * frame_size);
...
static ssize_t process_frames(struct omap_stream_in *in, void *buffer,
			      ssize_t frames)
{
	ssize_t frames_wr = 0;
	...
	while (frames_wr < frames) {
		...
		out_buf.s16 =
		    (int16_t *) buffer + frames_wr * in->config.channels;
		...
		if (in->proc_frames_in) {
			memcpy(in->proc_buf,
			       in->proc_buf +
			       in_buf.frameCount * in->config.channels,
				   ...
				   struct audio_stream_out **stream_out)
{
	...
	out =
	    (struct omap_stream_out *)calloc(1, sizeof(struct omap_stream_out));
	if (!out)
		return -ENOMEM;
...
static int adev_set_parameters(struct audio_hw_device *dev, const char *kvpairs)
{
	...
	parms = str_parms_create_str(kvpairs);
	ret =
	    str_parms_get_str(parms, AUDIO_PARAMETER_KEY_TTY_MODE, value,
			      sizeof(value));
	if (ret >= 0) {
		int tty_mode;
	...
	ret =
	    str_parms_get_str(parms, AUDIO_PARAMETER_KEY_BT_NREC, value,
			      sizeof(value));
	if (ret >= 0) {
...
static int adev_set_mic_mute(struct audio_hw_device *dev, bool state)
{
	...
	if (adev->mode == AUDIO_MODE_IN_CALL) {
		for (channel = 0;
		     channel <
		     mixer_ctl_get_num_values(adev->mixer_ctls.voice_ul_volume);
		     channel++) {
			mixer_ctl_set_value(adev->mixer_ctls.voice_ul_volume,
					    channel,
					    (state ? 0 : MIXER_ABE_GAIN_0DB));
		}
	} else {
		for (channel = 0;
		     channel <
		     mixer_ctl_get_num_values(adev->mixer_ctls.amic_ul_volume);
		     channel++) {
			mixer_ctl_set_value(adev->mixer_ctls.amic_ul_volume,
					    channel,
					    (state ? 0 : MIXER_ABE_GAIN_0DB));
		...
		for (channel = 0;
		     channel <
		     mixer_ctl_get_num_values(adev->mixer_ctls.dmic1_ul_volume);
		     channel++) {
		...
		for (channel = 0;
		     channel <
		     mixer_ctl_get_num_values(adev->mixer_ctls.dmic2_ul_volume);
		     channel++) {
		...
		for (channel = 0;
		     channel <
		     mixer_ctl_get_num_values(adev->mixer_ctls.dmic3_ul_volume);
		     channel++) {
		...
		for (channel = 0;
		     channel <
		     mixer_ctl_get_num_values(adev->mixer_ctls.bt_ul_volume);
		     channel++) {
			mixer_ctl_set_value(adev->mixer_ctls.bt_ul_volume,
					    channel,
					    (state ? 0 : DB_TO_ABE_GAIN(6)));
		}
	}
				  ...
				  struct audio_stream_in **stream_in)
{
	...
	*stream_in = NULL;
	...
	in->buffer =
	    malloc(2 * in->config.period_size *
		   audio_stream_frame_size(&in->stream.common));
	if (!in->buffer) {
		ret = -ENOMEM;
		goto err;
		     ...
		     hw_device_t ** device)
{
	...
	if (adev->mixer_ctls.mm_dl2 && adev->mixer_ctls.vx_dl2 &&
		...
		free(adev);
		ALOGE
		    ("Unable to identify ABE downlink capabilities, aborting.");
		return -EINVAL;
	}
	...
	if (!adev->mixer_ctls.mm_dl1 || !adev->mixer_ctls.vx_dl1 ||
	    ...
	    || !adev->mixer_ctls.dmic1_ul_volume || !adev->mixer_ctls.dl1_eq
	...
	if (adev->modem) {
		...
		ril_register_set_wb_amr_callback(audio_set_wb_amr_callback,
						 (void *)adev);
	}
